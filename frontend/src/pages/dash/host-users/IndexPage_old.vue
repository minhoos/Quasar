<template>
  <div class="q-pa-md q-pa-md-xl column items-start q-gutter-md" padding>
    ...
    <UserModal v-model="isModal" @showModal="closeModal"></UserModal>
  </div>
</template>
...
const fetchFromServer = (startRow, count, filter, sortBy, descending) => {
  const data = filter
    ? hostUsers.filter(row => row.name.includes(filter))
    : hostUsers.slice()
...
const getRowsNumberCount = (filter) => {
  if (!filter) {
    return hostUsers.length
  }
  let count = 0
  hostUsers.forEach(treat => {
...
onMounted(() => {
  // 테이블의 데이터를 가져온다.
  hostUser.value.requestServerInteraction();
})
